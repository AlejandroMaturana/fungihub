...
> **RTOS**: FreeRTOS (6 tareas en 2 núcleos)
...
firmware-esp32/
├── src/
│   ├── main.cpp               # Setup + 6 tareas FreeRTOS, variables compartidas volatile
...
│   ├── hysteresis_controller.h/.cpp  # Reglas locales T/H/CO₂, modos LOCAL/REMOTE/OFF
│   ├── state_machine.h/.cpp   # 8 estados + SWDT + safe mode + NVS
│   ├── ota_handler.h/.cpp     # ArduinoOTA + HTTP Update
...
├── partitions.csv             # Tabla de particiones OTA (app0/app1/otadata/nvs)
├── VERSION
└── package.json
```
...
|---|---|---|
| Loop principal | `main.cpp` | Setup + 6 tareas FreeRTOS, shared volatile |
...
| Histéresis | `hysteresis_controller.h/.cpp` | Reglas locales T/H/CO₂, modos LOCAL/REMOTE/OFF |
| State Machine | `state_machine.h/.cpp` | 8 estados + watchdog software + safe mode |
| OTA | `ota_handler.h/.cpp` | ArduinoOTA + HTTP Update |
...
| taskSSR | 1 | `configMAX_PRIORITIES-2` | 4096 | 250 | Histéresis + actuadores + SWDT |
...
| taskTelemetry | 0 | `configMAX_PRIORITIES-4` | 4096 | 5000 | ThingSpeak + stats |
...
3. **Ejecutor OTA** — Descarga, verifica hash, flashea en partición inactiva
4. **Confirmación Post-Boot** — Self-test → `mark_valid()` o rollback automático
