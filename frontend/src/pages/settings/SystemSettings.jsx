  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {[msg, seedMsg].filter(Boolean).map((m, i) => (
        <div key={i} style={{ padding: '10px 14px', borderRadius: '8px', background: m.type === 'ok' ? 'rgba(var(--spore-green-rgb), 0.08)' : 'rgba(239, 68, 68, 0.08)', border: `1px solid ${m.type === 'ok' ? 'rgba(var(--spore-green-rgb), 0.2)' : 'rgba(239, 68, 68, 0.2)'}` }}>
          ...
          {CATEGORY_ORDER.filter(c => grouped[c]).map(cat => (
            <div key={cat} className="glass-card" style={{ padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--on-surface-variant)' }}>{CATEGORY_LABELS[cat] || cat}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {grouped[cat].map(s => (
                  <div key={s.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', borderRadius: '8px', background: 'var(--surface-container)', border: '1px solid var(--outline-variant)' }}>
                    <div style={{ flex: 1, marginRight: '16px' }}>
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--outline)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block' }}>{s.label || s.key}</span>
      ...
      <div className="glass-card" style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--on-surface-variant)' }}>Telegram Bot</span>
        </div>

        {tgStatus && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '8px', background: tgStatus.running ? 'rgba(var(--spore-green-rgb), 0.08)' : 'rgba(239, 68, 68, 0.08)', border: `1px solid ${tgStatus.running ? 'rgba(var(--spore-green-rgb), 0.2)' : 'rgba(239, 68, 68, 0.2)'}`, marginBottom: '16px' }}>
        ...
        <form onSubmit={handleConfigureTelegram} style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '400px' }}>
          <div>
            <label style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--outline)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: '4px' }}>Bot Token</label>
          ...
          <div>
            <label style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--outline)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'block', marginBottom: '4px' }}>Bot Username</label>
        ...
        <details style={{ marginTop: '16px' }}>
          <summary style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', color: 'var(--outline)', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }}>How to create a Telegram bot?</summary>
